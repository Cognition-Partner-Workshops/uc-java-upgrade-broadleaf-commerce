...
public class VelocityMessageCreator extends MessageCreator {
    ...
    public String buildMessageBody(EmailInfo info, Map<String,Object> props) {
        ...
        if (props instanceof HashMap) {
            ...
            // TODO(java21-migration): Spring 6 removed org.springframework.ui.velocity.VelocityEngineUtils (Spring
            // dropped Velocity integration). Merge the template directly through the Velocity API to preserve behavior.
            StringWriter result = new StringWriter();
            try {
                velocityEngine.mergeTemplate(info.getEmailTemplate(), info.getEncoding(), new VelocityContext(propsCopy), result);
            } catch (Exception e) {
                throw new RuntimeException("Could not merge velocity email template " + info.getEmailTemplate(), e);
            }
