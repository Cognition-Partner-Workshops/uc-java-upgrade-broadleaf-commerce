...
public class BroadleafThymeleafMessageResolver extends AbstractMessageResolver {
    ...
    @Override
    public String resolveMessage(final ITemplateContext context, final Class<?> origin,
                                  final String key, final Object[] messageParameters) {
        if (context == null || context.getLocale() == null || key == null) {
            return null;
        }
        
        if (I18N_VALUE_KEY.equals(key) && messageParameters != null && messageParameters.length >= 2) {
