 ...
 *
 */
// TODO(java21-migration): Thymeleaf 3 removed the DOM-based processor model (org.thymeleaf.dom.*, org.thymeleaf.Arguments,
// AbstractElementProcessor, ProcessorResult). Element processors now extend AbstractElementTagProcessor and operate on
// the event-based model through IElementTagStructureHandler. Model variables are contributed as local variables
// (structureHandler.setLocalVariable) instead of mutating the expression-evaluation root Map, and the host element is
// removed via structureHandler.removeElement(). Precedence is now supplied to the constructor (getPrecedence() is final).
public abstract class AbstractModelVariableModifierProcessor extends AbstractElementTagProcessor {
    ...
    /**
     * This method will handle calling the modifyModelAttributes abstract method and remove the host element
     * from the template output.
     */
    @Override
    ...
    /**
     * Helper method to add a value to the model as a local variable
