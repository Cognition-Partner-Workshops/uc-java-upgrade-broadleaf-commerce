...
public abstract class AbstractModelVariableModifierProcessor extends AbstractElementTagProcessor {
    ...
    public AbstractModelVariableModifierProcessor(String elementName) {
        super(TemplateMode.HTML, "blc", elementName, true, null, false, 1000);
    }
    ...
    protected void doProcess(ITemplateContext context, IProcessableElementTag tag,
        ...
        modifyModelAttributes(context, tag);
        structureHandler.removeElement();
    }
    
    protected void addToModel(ITemplateContext context, String key, Object value) {
        structureHandler.setLocalVariable(key, value);
    }
    
    @SuppressWarnings("unchecked")
    protected <T> void addCollectionToExistingSet(ITemplateContext context, String key, Collection<T> value) {
        Object existing = context.getVariable(key);
        Set<T> items;
        if (existing instanceof Set) {
            items = new HashSet<T>((Set<T>) existing);
        } else {
            items = new HashSet<T>();
        }
        items.addAll(value);
    ...
    @SuppressWarnings("unchecked")
    protected <T> void addItemToExistingSet(ITemplateContext context, String key, Object value) {
        Object existing = context.getVariable(key);
        Set<T> items;
        if (existing instanceof Set) {
            items = new HashSet<T>((Set<T>) existing);
    ...
    protected abstract void modifyModelAttributes(ITemplateContext context, IProcessableElementTag element);
}
