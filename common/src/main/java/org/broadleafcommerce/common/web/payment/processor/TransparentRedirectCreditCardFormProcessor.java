...
public class TransparentRedirectCreditCardFormProcessor extends AbstractElementTagProcessor {
    ...
    public TransparentRedirectCreditCardFormProcessor() {
        super(TemplateMode.HTML, "blc", "transparent_credit_card_form", true, null, false, 1000);
    }
    ...
    protected void doProcess(ITemplateContext context, IProcessableElementTag tag, IElementTagStructureHandler structureHandler) {
        ...
        for (String attrName : tag.getAttributeMap().keySet()) {
            if (attrName.startsWith("config-")) {
                ...
                configurationSettings.put(configParam, tag.getAttributeValue(attrName));
            }
        }
        ...
        if (hiddenFields != null && !hiddenFields.isEmpty()) {
            for (Map.Entry<String, String> entry : hiddenFields.entrySet()) {
                Map<String, String> hiddenAttrs = new HashMap<String, String>();
                hiddenAttrs.put("type", "hidden");
                hiddenAttrs.put("name", entry.getKey());
                hiddenAttrs.put("value", entry.getValue());
                model.add(modelFactory.createStandaloneElementTag("input", hiddenAttrs, null, false, false));
            }
        }
